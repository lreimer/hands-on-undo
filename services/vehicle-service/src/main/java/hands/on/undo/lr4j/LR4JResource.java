...
public class LR4JResource {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(LR4JResource.class);
    ...
    public Response start() {
        try {
            ...
            LOGGER.warn("LR4J recording in progress.", recordInProgress);
            return Response.status(Response.Status.CONFLICT).build();
        } catch (NoAttachYama noAttachYama) {
            LOGGER.error("LR4J attachment error.", noAttachYama);
            return Response.serverError().build();
        } catch (APICallFailed apiCallFailed) {
            LOGGER.error("LR4J API call failed.", apiCallFailed);
            return Response.serverError().build();
        }
    ...
    public Response save(@PathParam("filename") @NotBlank String filename) {
        try {
            LOGGER.info("Saving LR4J recording to {}", filename);
    ...
    public Response get(@PathParam("filename") @NotBlank String filename) {
        LOGGER.info("Reading LR4J recording {}", filename);
